...
- ROSPlan stores the domain and current state in a persistent way decoupled from the planner. It allows replanning, changing the goals, and more flexibility in general. **To allow flexibility, the AOS should save its domain and current belief state outside the planner process.** 
